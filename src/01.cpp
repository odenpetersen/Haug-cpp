...
double BlackScholes(char call_put_flag, double stock_price, double strike_price, double time_to_expiry_years, double risk_free_rate_annual, double sigma_annual) {
	double d1 = (std::log(stock_price / strike_price) + (risk_free_rate_annual + std::pow(sigma_annual,2.) / 2.) * time_to_expiry_years) / (sigma_annual * std::sqrt(time_to_expiry_years));
	double d2 = d1 - sigma_annual * std::sqrt(time_to_expiry_years);

	if (call_put_flag == 'c') {
		return stock_price * pnorm(d1) - strike_price * std::exp( - risk_free_rate_annual * time_to_expiry_years) * pnorm(d2);
	} else if (call_put_flag == 'p') {
		return strike_price * std::exp( - risk_free_rate_annual * time_to_expiry_years) * pnorm( - d2) - stock_price * pnorm( - d1);
	} else {
...
double GBlackScholes(char call_put_flag, double stock_price, double strike_price, double time_to_expiry_years, double risk_free_rate_annual, double cost_of_carry_annual, double sigma_annual) {
	...
	if (call_put_flag == 'c') {
		return stock_price * std::exp((cost_of_carry_annual - risk_free_rate_annual) * time_to_expiry_years) * pnorm(d1) - strike_price * std::exp( - risk_free_rate_annual * time_to_expiry_years) * pnorm(d2);
	} else if (call_put_flag == 'p') {
		return strike_price * std::exp( - risk_free_rate_annual * time_to_expiry_years) * pnorm( - d2) - stock_price * std::exp((cost_of_carry_annual - risk_free_rate_annual) * time_to_expiry_years) * pnorm( - d1);
	} else {
...
void test_01() {
	...
	risk_free_rate_annual = 0.08;
	sigma_annual = 0.3;

	std::cout << "Correct BlackScholes output" << std::endl;
	expected = 2.1334;
	output = BlackScholes('c', stock_price, strike_price, time_to_expiry_years, risk_free_rate_annual, sigma_annual);
	...
	expected = output;
	output = GBlackScholes('c', stock_price, strike_price, time_to_expiry_years, risk_free_rate_annual, risk_free_rate_annual, sigma_annual);
	std::cout << expected << ' ' << output << ' ' << relative_error(expected,output) << std::endl;

	std::cout << "Put-Call Supersymmetry (Page 11)" << std::endl;
	output = BlackScholes('p', - stock_price, - strike_price, time_to_expiry_years, risk_free_rate_annual, - sigma_annual);
	std::cout << expected << ' ' << output << ' ' << relative_error(expected,output) << std::endl;
	...
	expected = 4.0870;
	output = GBlackScholes('p', stock_price, strike_price, time_to_expiry_years, risk_free_rate_annual, cost_of_carry_annual, sigma_annual);
	std::cout << expected << ' ' << output << ' ' << relative_error(expected,output) << std::endl;

	std::cout << "Put-Call Supersymmetry (Page 11)" << std::endl;
	expected = output;
	output = GBlackScholes('c', - stock_price, - strike_price, time_to_expiry_years, risk_free_rate_annual, cost_of_carry_annual, - sigma_annual);
	std::cout << expected << ' ' << output << ' ' << relative_error(expected,output) << std::endl;
